...
@pytest.fixture
def input_value():
    input = alpha  # You can change this to `beta` to test the bonus step
    return input
...
def test_length(input_value):
    ...
    """
    ### SOLUTION ###
    assert spellcheck.word_count(input_value) < 10  # Check if the number of words is less than 10
    assert spellcheck.char_count(input_value) < 50  # Check if the number of characters is less than 50
...
def test_struc(input_value):
    ...
    """
    ### SOLUTION ###
    assert spellcheck.first_char(input_value).isupper()  # Check if the first character is uppercase
    assert spellcheck.last_char(input_value) == "."      # Check if the last character is a period
